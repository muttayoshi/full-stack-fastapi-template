    ...
    with patch.object(settings, "GOOGLE_CLIENT_ID", None):
        with patch.object(settings, "GOOGLE_CLIENT_SECRET", None):
            response = client.post(
                "/api/v1/users/oauth/google", json={"code": "test_code"}
            )
...
def test_google_oauth_empty_code(client: TestClient) -> None:
    ...
    response = client.post("/api/v1/users/oauth/google", json={"code": ""})
...
async def test_google_oauth_invalid_code(client: TestClient) -> None:
    ...
    with patch.object(settings, "google_oauth_enabled", True):
        with patch.object(
            ...
            response = client.post(
                "/api/v1/users/oauth/google", json={"code": "invalid_code"}
            )
...
async def test_google_oauth_unverified_email(client: TestClient) -> None:
    ...
    with patch.object(settings, "google_oauth_enabled", True):
        with patch.object(
            ...
            response = client.post(
                "/api/v1/users/oauth/google", json={"code": "valid_code"}
            )
            assert response.status_code == 400
...
@pytest.mark.asyncio
async def test_google_oauth_success_new_user(client: TestClient) -> None:
...
@pytest.mark.asyncio
async def test_google_oauth_inactive_user(client: TestClient, db: Session) -> None:
...
class TestOAuthService:
    ...
    @pytest.mark.asyncio
    async def test_exchange_google_code_oauth_disabled(self, db: Session) -> None:
    ...
    def test_create_or_link_google_account_new_user(self, db: Session) -> None:
        """Test creating a new user from Google OAuth"""
        oauth_service = OAuthService(db)

        user = oauth_service.create_or_link_google_account(
            google_id="new_google_id", email="newuser@example.com", full_name="New User"
        )
    ...
    def test_create_or_link_google_account_existing_user(self, db: Session) -> None:
    ...
    def test_create_or_link_google_account_already_linked(self, db: Session) -> None:
